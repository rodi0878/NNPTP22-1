...
namespace NNPTPZ1.Mathematics.Tests
{
    ...
    public class CplxTests
    {
        ...
        public void AddTestPolynome()
        {
            ...
            polynome.Coefficients.Add(new ComplexNumber() { Real = 1, Imaginary = 0 });
            ComplexNumber result = polynome.Eval(new ComplexNumber() { Real = 0, Imaginary = 0 });
            var expected = new ComplexNumber() { Real = 1, Imaginary = 0 };
            Assert.AreEqual(expected, result);
            result = polynome.Eval(new ComplexNumber() { Real = 1, Imaginary = 0 });
            expected = new ComplexNumber() { Real = 2, Imaginary = 0 };
            Assert.AreEqual(expected, result);
            result = polynome.Eval(new ComplexNumber() { Real = 2, Imaginary = 0 });
